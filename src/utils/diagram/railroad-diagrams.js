...
export class FakeSVG {
  ...
  toSVG() {
    ...
    /**
     * Modified to support any type of children
     * @author Thiago Ausechi
     */
    if (Array.isArray(this.children))
      this.children.forEach(function (e) {
        if ('toSVG' in e) el.appendChild(e.toSVG());
        else el.textContent = e;
      });
    else el.textContent = this.children;
...
export class DiagramMultiContainer extends FakeSVG {
  constructor(tagName, items, attrs, text) {
    ...
    if (flattenedItems.length === 0) this.items = [new Comment('Regra vazia!')];
    else this.items = flattenedItems.map(wrapString);
  }
  walk(cb) {
    cb(this);
